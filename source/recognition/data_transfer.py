...
def decodeToBytes(RS_ACGT_str_lists,Data_seq_cnt,chunk_size):
    # 1.转换为01序列
...
def decodeToText(RS_ACGT_str_lists,Data_seq_cnt,chunk_size=57):
    bytes_str = decodeToBytes(RS_ACGT_str_lists,Data_seq_cnt,chunk_size)
    # 5.尝试恢复原始数据
    try:
...
## 从ACGT碱基序列转换为原数据内容
def decodeToImage(output_path,RS_ACGT_str_lists,Data_seq_cnt,chunk_size, image_format='PNG'):
    print("decodeToImage: 序列重构后后每个序列长", len(RS_ACGT_str_lists[0]),"序列数目：", len(RS_ACGT_str_lists))

    # 2.将字节流转换numpy数组
    byte_array = decodeToBytes(RS_ACGT_str_lists, Data_seq_cnt, chunk_size)
...
## 从ACGT碱基序列转换为原数据内容
def decodeToVedio(output_vedio_path,RS_ACGT_str_lists,Data_seq_cnt,chunk_size, image_format='PNG'):
    print("decodeToVedio: 序列重构后后每个序列长", len(RS_ACGT_str_lists[0]),"序列数目：", len(RS_ACGT_str_lists))
    bytes_arr = decodeToBytes(RS_ACGT_str_lists, Data_seq_cnt, chunk_size)
